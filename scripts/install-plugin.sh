...
LOCAL_MANIFEST="$REPO_ROOT/apps/plugin/dist-obsidian/manifest.json"

mkdir -p "$TARGET_DIR"

if [ -f "$LOCAL_MAIN" ] && [ -f "$LOCAL_MANIFEST" ]; then
  cp "$LOCAL_MAIN" "$TARGET_DIR/main.js"
  cp "$LOCAL_MANIFEST" "$TARGET_DIR/manifest.json"
else
  ...
  curl -fsSL "$RAW_BASE/apps/plugin/dist-obsidian/manifest.json" -o "$TARGET_DIR/manifest.json"
fi
