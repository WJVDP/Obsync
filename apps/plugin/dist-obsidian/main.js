...
// src/event-capture/vaultEventCapture.ts
var VaultEventCapture = class {
  constructor() {
...
// src/crdt-engine/yjsEngine.ts
var YjsMarkdownEngine = class {
  constructor() {
  ...
  applyText(filePath, text2) {
    const doc2 = this.getOrCreate(filePath);
    const yText = doc2.getText("content");
    yText.delete(0, yText.length);
    yText.insert(0, text2);
    return {
      updateBase64: Buffer.from(encodeStateAsUpdate(doc2)).toString("base64"),
      stateVectorBase64: Buffer.from(encodeStateVector(doc2)).toString("base64")
    };
  }
  mergeRemoteUpdate(filePath, remoteUpdateBase64) {
    const doc2 = this.getOrCreate(filePath);
...
// src/transport/syncTransport.ts
var SyncTransport = class {
  ...
  async request(path, init) {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Request failed (${response.status}): ${text2}`);
    }
    return await response.json();
  }
  async push(vaultId, request) {
    return this.request(`/v1/vaults/${vaultId}/sync/push`, {
      method: "POST",
      body: JSON.stringify(request)
    });
  }
  async pull(vaultId, since, deviceId) {
...
// src/state-store/stateStore.ts
var SyncStateStore = class {
  constructor(persistence) {
    this.persistence = persistence;
    __publicField(this, "outbox", []);
    __publicField(this, "cursors", /* @__PURE__ */ new Map());
    __publicField(this, "keyEnvelopes", /* @__PURE__ */ new Map());
  }
  async load() {
    if (!this.persistence) {
      return;
  ...
  async setKeyEnvelopes(vaultId, envelopes) {
    this.keyEnvelopes.set(vaultId, envelopes);
    await this.persist();
  }
  getKeyEnvelopes(vaultId) {
    return this.keyEnvelopes.get(vaultId) ?? [];
  }
};

// src/op-builder/syncOpBuilder.ts
function inferOpType(event) {
...
var SyncOpBuilder = class {
  constructor(deviceId) {
    this.deviceId = deviceId;
    __publicField(this, "logicalClock", 0);
  }
  fromVaultEvent(event, fileId) {
    this.logicalClock += 1;
    return {
      idempotencyKey: v4_default(),
...
var SyncEngine = class {
  constructor(options, transport, stateStore, crdtEngine, telemetry) {
    this.options = options;
    ...
    __publicField(this, "realtimeSocket", null);
    __publicField(this, "opBuilder");
  ...
  buildOperation(event) {
    ...
    return this.opBuilder.fromVaultEvent(event, void 0);
  }
  async flushOutbox() {
...
function isRealtimeEvent(payload) {
  ...
  const value = payload;
  return value.type === "event" && typeof value.seq === "number" && typeof value.opType === "string" && typeof value.payload === "object" && value.payload !== null;
}
  ...
  registerVaultEventHandlers() {
    ...
    this.registerEvent(
      this.app.vault.on("rename", async (file, oldPath) => {
        const content = file instanceof import_obsidian.TFile ? await this.readMarkdown(file) : void 0;
        this.eventCapture.emit({
          type: "rename",
          path: file.path,
          oldPath,
...
var ObsyncSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Vault ID").setDesc("Server vault id").addText(
      (text2) => text2.setValue(this.plugin.settings.vaultId).onChange(async (value) => {
        this.plugin.settings.vaultId = value.trim();
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Device ID").setDesc("Stable device identifier").addText(
      (text2) => text2.setValue(this.plugin.settings.deviceId).onChange(async (value) => {
        this.plugin.settings.deviceId = value.trim() || v4_default();
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("API Token").setDesc("Preferred auth for automation").addText(
      (text2) => text2.setValue(this.plugin.settings.apiToken).onChange(async (value) => {
        this.plugin.settings.apiToken = value.trim();
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Email").setDesc("Used when API token is empty").addText(
      (text2) => text2.setValue(this.plugin.settings.email).onChange(async (value) => {
        this.plugin.settings.email = value.trim();
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Password").setDesc("Used when API token is empty").addText((text2) => {
      text2.inputEl.type = "password";
      return text2.setValue(this.plugin.settings.password).onChange(async (value) => {
        this.plugin.settings.password = value;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian.Setting(containerEl).setName("Realtime").setDesc("Open websocket stream after connect").addToggle(
    ...
    new import_obsidian.Setting(containerEl).setName("Auto connect on load").setDesc("Connect when Obsidian starts").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.autoConnectOnLoad).onChange(async (value) => {
        this.plugin.settings.autoConnectOnLoad = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Connect now").setDesc("Validate config and start sync").addButton(
