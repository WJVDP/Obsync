...
var ScopeSchema = external_exports.enum(["read", "write", "admin"]);
var OpTypeSchema = external_exports.enum([
  "md_update",
  "file_create",
  "file_rename",
  "file_delete",
...
var SyncOpSchema = external_exports.object({
  idempotencyKey: external_exports.string().min(8),
  deviceId: external_exports.string().uuid(),
  fileId: external_exports.string().uuid().optional(),
  path: external_exports.string().min(1),
  opType: OpTypeSchema,
  logicalClock: external_exports.number().int().nonnegative(),
  payload: external_exports.record(external_exports.any()),
  createdAt: external_exports.string().datetime()
});
var SyncBatchPushRequestSchema = external_exports.object({
  deviceId: external_exports.string().uuid(),
  cursor: external_exports.number().int().nonnegative().default(0),
  ops: external_exports.array(SyncOpSchema).min(1)
});
var SyncBatchPushResponseSchema = external_exports.object({
  acknowledgedSeq: external_exports.number().int().nonnegative(),
  appliedCount: external_exports.number().int().nonnegative(),
  missingChunks: external_exports.array(external_exports.object({ blobHash: external_exports.string(), index: external_exports.number().int().nonnegative() })).default([]),
  rebaseRequired: external_exports.boolean().default(false)
});
var PullResponseSchema = external_exports.object({
  watermark: external_exports.number().int().nonnegative(),
  ops: external_exports.array(external_exports.object({
    seq: external_exports.number().int().nonnegative(),
    vaultId: external_exports.string().uuid(),
    fileId: external_exports.string().uuid().nullable(),
    opType: OpTypeSchema,
    payload: external_exports.record(external_exports.any()),
    createdAt: external_exports.string().datetime()
  }))
});
var BlobInitRequestSchema = external_exports.object({
  hash: external_exports.string().min(32),
  size: external_exports.number().int().positive(),
  chunkCount: external_exports.number().int().positive(),
  cipherAlg: external_exports.string().default("AES-256-GCM")
});
var BlobCommitRequestSchema = external_exports.object({
  hash: external_exports.string().min(32),
  expectedChunkCount: external_exports.number().int().positive(),
  expectedSize: external_exports.number().int().positive()
});
var ConflictRecordSchema = external_exports.object({
  ...
  createdAt: external_exports.string().datetime(),
  resolution: external_exports.enum(["pending", "restored", "ignored"]).default("pending")
});
var VaultStatusSchema = external_exports.object({
  vaultId: external_exports.string().uuid(),
  latestSeq: external_exports.number().int().nonnegative(),
  activeDevices: external_exports.number().int().nonnegative(),
  pendingBlobCount: external_exports.number().int().nonnegative(),
...
// ../../packages/shared/dist/crypto.js
...
function encryptVaultKeyForDevice(vaultKey, devicePublicKeyPem) {
...
function decryptVaultKeyForDevice(encryptedVaultKeyBase64, devicePrivateKeyPem) {
...
function decryptPayload(cipherText, key) {
  const decipher = (0, import_node_crypto.createDecipheriv)("aes-256-gcm", key, Buffer.from(cipherText.ivBase64, "base64"));
...
// ../../packages/shared/dist/chunking.js
var import_node_crypto2 = require("node:crypto");
function chunkBuffer(buffer, chunkSizeBytes = 1024 * 1024) {
  if (chunkSizeBytes <= 0) {
  ...
  for (let offset = 0, index = 0; offset < buffer.length; offset += chunkSizeBytes, index += 1) {
    ...
    chunks.push({
      index,
      data,
      hash: (0, import_node_crypto2.createHash)("sha256").update(data).digest("hex")
    });
  }
  return chunks;
}
...
var SyncTransport = class {
  ...
  async registerDevice(vaultId, request) {
    await this.request(`/v1/vaults/${vaultId}/devices/register`, {
      method: "POST",
      body: JSON.stringify(request)
    });
  }
  async getKeyEnvelopes(vaultId, deviceId) {
    const params2 = new URLSearchParams({ deviceId });
    const response = await this.request(
      `/v1/vaults/${vaultId}/keys?${params2.toString()}`
    );
    return response.envelopes ?? [];
  }
  async rotateKeys(vaultId, version, envelopes) {
    await this.request(`/v1/vaults/${vaultId}/keys/rotate`, {
      method: "POST",
      body: JSON.stringify({ version, envelopes })
    });
  }
  async initBlob(vaultId, payload) {
    return this.request(`/v1/vaults/${vaultId}/blobs/init`, {
      method: "POST",
      body: JSON.stringify(payload)
    });
  }
  async uploadBlobChunk(vaultId, blobHash, index, payload) {
    await this.request(
  ...
  async commitBlob(vaultId, blobHash, expectedChunkCount, expectedSize) {
    await this.request(`/v1/vaults/${vaultId}/blobs/${blobHash}/commit`, {
      method: "POST",
      body: JSON.stringify({
        hash: blobHash,
        expectedChunkCount,
  ...
  async getBlobManifest(vaultId, blobHash) {
    return this.request(`/v1/vaults/${vaultId}/blobs/${blobHash}`);
  }
  async getBlobChunk(vaultId, blobHash, index) {
    const response = await this.request(`/v1/vaults/${vaultId}/blobs/${blobHash}/chunks/${index}`);
  ...
  openRealtime(vaultId, since, handlers) {
    ...
    const socket = new WebSocket(
      `${endpoint}/v1/vaults/${vaultId}/realtime?${params2.toString()}`,
...
var SyncStateStore = class {
  constructor(persistence) {
    ...
    __publicField(this, "vaultKeys", /* @__PURE__ */ new Map());
  }
  async load() {
    if (!this.persistence) {
      return;
  ...
  async setVaultKey(vaultId, keyBase64) {
...
// src/blob-engine/blobSyncEngine.ts
var BlobSyncEngine = class {
  planUpload(rawData, vaultKey, chunkSizeBytes = 1024 * 1024) {
  ...
  decryptBlob(encryptedBlob, vaultKey, ivBase64, authTagBase64) {
...
var SyncEngine = class {
  constructor(options, transport, stateStore, crdtEngine, telemetry) {
    ...
    __publicField(this, "blobEngine", new BlobSyncEngine());
  ...
  async buildOperation(event) {
    ...
    if ((event.type === "create" || event.type === "modify") && !event.path.endsWith(".md") && event.binaryContentBase64) {
  ...
  async bootstrapKeys() {
    ...
    const latestEnvelope = envelopes.reduce(
      (acc, current) => current.version > acc.version ? current : acc,
      envelopes[0]
    );
    if (latestEnvelope) {
  ...
  async applyRemoteOp(opType, payload) {
    ...
    if (opType === "file_create") {
      ...
      const content = typeof payload.content === "string" ? payload.content : void 0;
...
// src/conflict-ui/conflictPresenter.ts
var ConsoleConflictPresenter = class {
  show(record) {
...
var ObsyncPlugin = class extends import_obsidian.Plugin {
  constructor() {
    ...
    __publicField(this, "conflictPresenter", new ConsoleConflictPresenter());
  ...
  registerVaultEventHandlers() {
    ...
    this.registerEvent(
      this.app.vault.on("rename", async (file, oldPath) => {
        const payload = file instanceof import_obsidian.TFile ? await this.readFilePayload(file) : { content: void 0 };
        this.eventCapture.emit({
          type: "rename",
          path: file.path,
          oldPath,
  ...
  async applyRemoteFileCreate(path, content) {
    this.markPathSuppressed(path);
    await this.ensureFolderForPath(path);
    const existing = this.app.vault.getAbstractFileByPath(path);
    if (!existing) {
  ...
  async applyRemoteFileRename(oldPath, newPath) {
    const source = this.app.vault.getAbstractFileByPath(oldPath);
    if (!source || !(source instanceof import_obsidian.TFile || source instanceof import_obsidian.TFolder)) {
      return;
    }
  ...
  async applyRemoteFileDelete(path) {
    const existing = this.app.vault.getAbstractFileByPath(path);
    if (!existing) {
      return;
    }
  ...
  async applyRemoteBinaryFile(path, content) {
    this.markPathSuppressed(path);
    await this.ensureFolderForPath(path);
  ...
  async allocateConflictPath(path) {
  ...
  reportConflict(path, reason) {
    ...
    new import_obsidian.Notice(`Obsync conflict: ${path}`);
  }
  startRealtime() {
    if (!this.syncEngine) {
      return;
